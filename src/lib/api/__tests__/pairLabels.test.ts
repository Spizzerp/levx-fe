...
import {
  KNOWN_DEVNET_MINTS,
  KNOWN_DEVNET_QUOTE_MINTS,
  resolveBaseMintLabel,
  resolvePairLabel,
} from '@/lib/api/pairLabels'
...
describe('resolveBaseMintLabel', () => {
  ...
  it('resolves the devnet wrapped-BTC mint to BTC/USDC', () => {
    const label = resolveBaseMintLabel(
...
describe('resolvePairLabel', () => {
  it('uses the explicit quote mint when it differs from the canonical USDC pair', () => {
    const label = resolvePairLabel(
      'So11111111111111111111111111111111111111112',
      'BPFLoader1111111111111111111111111111111111',
    )
    expect(label).toEqual({ pair: 'SOL/BPFL…', base: 'SOL', quote: 'BPFL…' })
  })

  it('keeps the canonical label when the known quote mint is present', () => {
    const label = resolvePairLabel(
      'So11111111111111111111111111111111111111112',
      Object.keys(KNOWN_DEVNET_QUOTE_MINTS)[0],
    )
    expect(label).toEqual({ pair: 'SOL/USDC', base: 'SOL', quote: 'USDC' })
  })
})
