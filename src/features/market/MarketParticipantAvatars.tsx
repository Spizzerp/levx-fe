...
import { useMarketParticipants } from '@/features/market/useMarketParticipants'
...
export function MarketParticipantAvatars({
  ...
  const { data } = useMarketParticipants(traderCount > 0 ? marketIdNum : null)
  const totalParticipants = data?.totalParticipants ?? 0

  const visibleParticipants = useMemo(() => {
    return (data?.participants ?? []).slice(0, MAX_VISIBLE_AVATARS)
  }, [data])

  const wallets = useMemo(() => {
    return visibleParticipants.map((participant) => participant.wallet)
  }, [visibleParticipants])

  const { data: profileMap } = useProfiles(wallets)

  const avatars = useMemo<AvatarCircle[]>(() => {
    return visibleParticipants.map((participant) => {
      const profile = profileMap?.[participant.wallet]
      return {
        id: participant.wallet,
        label: displayNameFor(participant.wallet, profile),
        wallet: participant.wallet,
        username: profile?.username,
        pnl: participant.pnl,
        exposure: participant.exposure,
        imageUrl: profileImageFor(profile),
        fallback: sigilAvatar(participant.wallet, profile),
      }
    })
  }, [profileMap, visibleParticipants])

  if (traderCount <= 0 || totalParticipants <= 0 || avatars.length === 0) return null
